-- 🚀 TUTORA PRODUCTION DATABASE SCHEMA
-- This schema supports a multi-tenant SaaS platform with full feature set
...
CREATE POLICY "Modules are isolated by company" ON modules
    FOR ALL USING (company_id = auth.jwt() ->> 'company_id'::text);

CREATE POLICY "Module assignments are isolated by company" ON module_assignments
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM modules m 
            WHERE m.id = module_assignments.module_id 
            AND m.company_id = auth.jwt() ->> 'company_id'::text
        )
    );

-- User-specific policies
CREATE POLICY "Users can only access their own assignments" ON module_assignments
...
-- Insert sample user (will be replaced by auth.users)
INSERT INTO users (email, name, company_id, role, position, points, streak_days)
VALUES ('demo@tutora.com', 'Demo User', (SELECT id FROM companies LIMIT 1), 'admin', 'Manager', 500, 7)
ON CONFLICT DO NOTHING; 