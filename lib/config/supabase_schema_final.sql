-- 🚀 TUTORA FINAL DATABASE SCHEMA
-- This script safely adds missing columns and creates all necessary objects
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'max_ai_modules') THEN
        ALTER TABLE companies ADD COLUMN max_ai_modules INTEGER DEFAULT 10;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'max_storage_gb') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'branding') THEN
...
-- Create module_assignments table if it doesn't exist
...
-- Success message
SELECT 'Database schema deployed successfully! All tables, columns, triggers, and policies are now ready.' as status; 