...
class _UserStatsScreenState extends State<UserStatsScreen> {
  ...
  void _filterUsers() {
    ...
    List<UserModel> result = _users.where((user) {
      ...
      final positionMatch = (user.position ?? '').toLowerCase().contains(
        _searchQuery.toLowerCase(),
      );
    ...
    if (_selectedFilter != 'All Users') {
      switch (_selectedFilter) {
        ...
        case 'Least Progress':
          result.sort(
            (a, b) =>
                (a.modulesCompleted ?? 0).compareTo(b.modulesCompleted ?? 0),
          );
          break;
        case 'New Users':
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('User Statistics'), centerTitle: true),
      body: _isLoading
