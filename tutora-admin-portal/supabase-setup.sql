-- 🚀 TUTORA SUPABASE DATABASE SETUP
...
-- Companies: Only service role can insert/update companies (for registration)
...
-- Modules: Admins can manage modules in their company
CREATE POLICY "Admins can manage company modules" ON modules
  FOR ALL USING (
    company_id IN (
...
-- Analytics: Service role can insert analytics
...
DO $$
BEGIN
    RAISE NOTICE 'SUCCESS: Tutora database setup completed!';
    RAISE NOTICE 'Created 5 tables: companies, users, modules, support_tickets, analytics';
    RAISE NOTICE 'Added 2 demo companies for testing';
