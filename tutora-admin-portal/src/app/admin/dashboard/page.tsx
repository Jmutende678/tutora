    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setStats({
          ...
          totalModules: data.companies.reduce((sum: number, company: any) => sum + (company.modules || 0), 0),
          completedModules: Math.round(data.users.total * (data.metrics.completionRate / 100)),
          pendingTickets: 0, // Will be fetched from support system
          monthlyRevenue: Math.round(data.revenue.monthly),
          aiModulesCreated: Math.round(data.users.total * 0.1), // Estimate based on user activity
          completionRate: data.metrics.completionRate,
          userGrowth: data.users.growth,
          moduleEngagement: data.metrics.engagement
        })

        // Set real companies data
        ...
        console.log('✅ Real admin dashboard data loaded:', {
          totalUsers: data.users.total,
