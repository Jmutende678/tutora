...
export default function PricingPage() {
  ...
  const handleTestBasic = async () => {
    setIsLoading('test')
    try {
      const response = await fetch('/api/stripe-test-basic', {
        method: 'POST',
  ...
  const handleTestFresh = async () => {
    setIsLoading('fresh')
    try {
      const response = await fetch('/api/stripe-create-fresh', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      })
      ...
      if (result.success && result.url) {
        ...
        alert(`Fresh test failed: ${result.error}`)
      }
    } catch (error) {
      alert(`Fresh test error: ${error}`)
    } finally {
      setIsLoading(null)
    }
  }
    ...
    try {
      ...
      const response = await fetch('/api/stripe/create-checkout-session', {
        ...
        body: JSON.stringify({
          planId,
          billingCycle,
          userCount: plan.baseUsers, // Default to base users for now
      ...
      alert(`Error starting trial: ${errorMessage}. Please try again or contact support.`)
    } finally {
      setIsLoading(null)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="pt-32 pb-16">
        {/* Test Button */}
        <div className="text-center py-4 bg-yellow-100 mb-8">
          <button 
          ...
          >
            {isLoading === 'test' ? 'Testing...' : 'TEST BASIC STRIPE'}
          </button>
          <button 
            onClick={handleTestFresh}
            disabled={isLoading === 'fresh'}
            className="bg-green-600 text-white px-4 py-2 rounded font-bold hover:bg-green-700"
          >
            {isLoading === 'fresh' ? 'Creating...' : 'TEST FRESH PRICE (Try This!)'}
          </button>
        </div>
