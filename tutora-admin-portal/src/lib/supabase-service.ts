import { supabaseAdmin, generateCompanyCode, Database, isSupabaseConfigured } from './supabase'
...
export class SupabaseService {
  ...
  private ensureSupabaseConfigured() {
    if (!isSupabaseConfigured() || !supabaseAdmin) {
      throw new Error('Database service not configured - please set up Supabase environment variables')
    }
  ...
  async createCompany(companyData: CreateCompanyRequest): Promise<Company> {
    try {
      const supabase = this.ensureSupabaseConfigured()
  ...
  async createUser(userData: CreateUserRequest): Promise<User> {
    try {
      // Create auth user
      const { data: authUser, error: authError } = await this.ensureSupabaseConfigured().auth.admin.createUser({
      ...
      // Create user record
      const { data: user, error: userError } = await this.ensureSupabaseConfigured()
        .from('users')
        .insert({
          id: authUser.user.id,
  ...
  async getUsersByCompany(companyId: string): Promise<User[]> {
    try {
      const { data: users, error } = await this.ensureSupabaseConfigured()
        .from('users')
        .select('*')
  ...
  async createSupportTicket(ticketData: CreateSupportTicketRequest): Promise<SupportTicket> {
    try {
      ...
      const { data: ticket, error } = await this.ensureSupabaseConfigured()
  ...
  async getModulesForCompany(companyId: string): Promise<Module[]> {
    try {
      const { data: modules, error } = await this.ensureSupabaseConfigured()
        .from('modules')
        .select('*')
        .eq('company_id', companyId)
        .eq('is_active', true)
  ...
  async getModuleById(moduleId: string): Promise<Module | null> {
    try {
      const { data: module, error } = await this.ensureSupabaseConfigured()
        .from('modules')
        .select('*')
        .eq('id', moduleId)
        .single()
  ...
  async createNotification(notificationData: {
    ...
    try {
      ...
      const { data: notification, error } = await this.ensureSupabaseConfigured()
        .from('notifications')
        .insert({
          id: nanoid(),
  ...
  async markNotificationsAsRead(notificationIds: string[]): Promise<number> {
    try {
      const { data, error } = await this.ensureSupabaseConfigured()
        .from('notifications')
        .update({ 
          is_read: true,
  ...
  async getAnalytics(companyId: string): Promise<AnalyticsData> {
    try {
      // Get user count
      const { count: totalUsers } = await this.ensureSupabaseConfigured()
        .from('users')
      ...
      const { count: activeUsers } = await this.ensureSupabaseConfigured()
        .from('users')
        .select('*', { count: 'exact', head: true })
        .eq('company_id', companyId)
      ...
      const { data: analytics, error } = await this.ensureSupabaseConfigured()
        .from('analytics')
  ...
  private async updateCompanyUserCount(companyId: string): Promise<void> {
    try {
      const { count } = await this.ensureSupabaseConfigured()
        .from('users')
        .select('*', { count: 'exact', head: true })
        .eq('company_id', companyId)
        .eq('is_active', true)

      await this.ensureSupabaseConfigured()
        .from('companies')
        .update({
